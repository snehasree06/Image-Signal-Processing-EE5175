...
lambda_end = 2.0;
lambda_step = 0.01;
n_lambdas = length(lambda_start:lambda_step:lambda_end);

%% L2 regularized NBD
sigma_n_vals = [8 8 8 5 10 15];
sigma_b_vals = [0.5 1 1.5 1 1 1]; %total 6 pairs of sigma_b and sigma_n combinations as per question
...
for i = 1:n_pairs
        ...
        [lambda_best, rmse] = best_lambda(lena, sigma_b, sigma_n, lambda_start, lambda_end, lambda_step, false, "L2");
        best_lambdas(i) = lambda_best;
        rmse_vals(i, :) = rmse;
        ...
        imshow(deblur_L2(degraded_image, sigma_b, lambda_best, qx, qy, false))
        name = ['Restored: \sigma_{b} = ', num2str(sigma_b), ' \sigma_{n} = ', num2str(sigma_n), ' \lambda = ', num2str(lambda_best)];
        title(name);
        saveas(fig1, ['L2_best_img_sigma_b_', num2str(10*sigma_b), '_sigma_n_', num2str(sigma_n)], 'png');
        fig2 = figure('visible', 'off');
        plot(lambda_start:lambda_step:lambda_end, rmse);
        grid on;
...
for i = 1:n_pairs
        ...
        imshow(deblur_L2(degrade(lena, sigma_b_vals(i), sigma_n_vals(i), false) , sigma_b_vals(i), best_looking_lambdas(i), qx, qy, false));
...
fig = figure('visible', 'off');
imshow(deblur_L2(degraded_1, sigma_b, 0.005, qx, qy, false));
...
fig = figure('visible', 'off');
imshow(deblur_L2(degraded_5, sigma_b, 0.2, qx, qy, false));
...
fig = figure('visible', 'off');
imshow(deblur_L2(degraded_mb, motion_blur_kernel, 0.07, qx, qy, true));
